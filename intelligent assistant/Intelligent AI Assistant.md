...
In our scenario, we will build agents for Dock Status, Surplus Handling and Secretary functions, all connected to a "routing agent" that will take requests from the end user and then select the appropriate agent for excecution before returning the answer. The flow of interacion we are simulating is one where a Warehouse Manager is asking for dock status, requests for recommendations on the handling of any surplus product, and notifies the right set of stakeholders around the warehouse.
...
Note: We will not use the traffic agent as this would require to deploy a langgraph agent in watsonx.ai first and would be beyond the scope of this bootcamp. However, of course it is possible to write agents in Langgraph, Crew.ai, etc and then deploy them as external agents in watsonx.ai as ai service and then import them into orchestrate!
...
### Pre-requisites
- Go to https://techzone.ibm.com/my/workshops/student/68f79bcca62483ab9217f335
...
Let's now test the new agent. In the Preview window, enter a question for the agent, for example "Can you tell me about the status of the warehouse docks?". If the agent is uncertain which specific dock you are interested in, then you can answer with "all docks".
 Note how the agent is using the provided data to formulate its answer.
...
In the following view, make sure to change the AI model to `llama-3-50b-instruct`.
![alt text](images/image_change_model.png)
...
```
what products do we have for truck with truck ID T001?

```

and if it asks for which specific details you want, you can answer with "all details". 
...
Let's test it. So, we are asking the questions as before and see if the answers are similar to the answers from the Surplus Agent and the Secretary Agent.

So, first we ask
`what products do we have for truck with truck ID T001? 
`, which should be transferred to the DockStatusAgent.

![alt text](images/warehouse_agent_surplus_agent.png)

Note that the agent was "reasoning", in other words, determining how to answer this request. It decided that routing the request to the DockStatusAgent was the best option.  You can expand the `Show reasoning` section in the Preview and see which steps the agent took. It should list one step, which you can expand as well.

![alt text](images/reasoning_preview.png)

We can test the same for writing an email with the Secretary agent by asking

`
Generate a notification email for the marketing team for SKU: 199464599 of 10 units
`

![alt text](images/warehouse_agent_secretary_agent.png)

Note how it selected the DockStatusAgent to answer the request, which was obviously the right choice in this case.
...
Now you can enter your questions and requests in the main chat window. Note that if not preselected, you first have to select the Warehouse Manager agent under the Agents list on the left (it is the only agent appearing in that list anyway). However, also note that you are not seeing the other agens like the DockStatusAgent or the SecretaryAgent as you configured to only interact with the Warehouse Manager Agent! You can use the same questions you asked when testing the individual agents above.
